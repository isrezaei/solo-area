...
export const AlbumsInfo = () => {
  const { images, type, name, artists, release_date, total_tracks } =
    useRecoilValue(NEW_RELEASES_ALBUMS_TRACK_ATOM);
  ...
  const { data } = useSWR(
    ["api", "GET_TRACKS_OF_NEW_RELEASES", new_releases],
    async (api, key, new_releases) =>
      await FETCH_NEW_RELESES_TRACK(new_releases),
  ...
  return (
    ...
    >
      ...
      <Flex height={"20vw"} direction={"column"} justify={"end"}>
        <Text color={"whiteAlpha.900"} fontSize={"1vw"} fontWeight={"bold"}>
          {type}
        </Text>
        <Text color={"whiteAlpha.900"} fontSize={"5vw"} fontWeight={"bold"}>
          {name}
        </Text>
