...
import {
  MY_PLAY_LIST_ID_ATOM,
  SPOTIFY_DEVICE_ID_ATOM,
  SPOTIFY_TRACKS_ID_ATOM,
} from "../atoms/atoms";
...
export const FeaturedPlayList = () => {
  ...
  const { data: PLAYLIST_DATA } = useSWR("FETCH FEATURED PLAYLIST", () =>
    FETCH_ONE_PLAYLIST(GET_PLAY_LIST_ID)
  );
  ...
  return (
    <Flex w={"full"} h={"100vh"} overflow={"hidden"} direction={"column"}>
      <Flex
        ...
        py={8}
        bgGradient={"linear(to-br , pink.900  , black , black )"}
      >
        <Image
          src={PLAYLIST_DATA?.images[0]?.url}
        ...
        >
          ...
          <Text fontSize={{ base: 30, md: 50 }} fontWeight={"bold"}>
            {PLAYLIST_DATA?.name}
          </Text>
      ...
      {PLAYLIST_DATA?.id === GET_PLAY_LIST_ID ? (
        <Divider my={3} bg={"whiteAlpha.700"} p={0.4} />
      ) : (
        <Progress my={3} size="xs" isIndeterminate />
      )}
            ...
            <Box w={"full"} key={TRACKS.track.id}>
              ...
              >
                ...
                >
                  ...
                  >
                    {" "}
                    {prettyMilliseconds(TRACKS?.track?.duration_ms, {
                      secondsDecimalDigits: 0,
                      colonNotation: true,
                    })}
                  </Text>
                  <Text display={{ base: "none", md: "block" }} flex={0.2}>
                    {trackID === TRACKS.track.id ? "play" : ""}
                  </Text>
                </Flex>
              </Tilt>
            </Box>
